...
def test_pre_h2_database_preserved_through_migration(monkeypatch, pg_url):
    ...
    with legacy_engine.begin() as c:
        ...
        # organization_id đã có từ schema baseline (b01) nên một DB l11 thật luôn có cột này;
        # migration t19 backfill reporting_unit_id dựa vào nó.
        c.execute(text("CREATE TABLE declarations (id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, reference_no VARCHAR UNIQUE, organization_id INTEGER, company_name VARCHAR, vessel_name VARCHAR)"))
        ...
        c.execute(text("INSERT INTO declarations(id,reference_no,organization_id,company_name,vessel_name) VALUES (9,'REF-9',7,'Legacy Co','Legacy Vessel')"))
