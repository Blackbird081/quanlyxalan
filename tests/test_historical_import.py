...
migration, real SQLite foreign-key enforcement and the service validators.
...
_db_path = Path(_tmp_dir) / "test_historical.db"
os.environ.setdefault("TEST_DATABASE_URL", f"sqlite:///{_db_path}")

# Importing backend.database registers the global SQLite foreign-key connect hook.
...
def test_fresh_database_upgrades_to_single_head(monkeypatch, tmp_path):
    ...
    assert heads == ["m12f0f000012"], f"expected a single head, got {heads}"
...
def test_pre_h2_database_preserved_through_migration(monkeypatch, tmp_path):
    ...
    with legacy_engine.begin() as c:
        c.execute(text("CREATE TABLE organizations (id INTEGER PRIMARY KEY, name VARCHAR NOT NULL UNIQUE, created_at VARCHAR, updated_at VARCHAR)"))
        c.execute(text("CREATE TABLE users (id INTEGER PRIMARY KEY, username VARCHAR NOT NULL UNIQUE, password_hash VARCHAR, role VARCHAR, organization_id INTEGER, is_active INTEGER, created_at VARCHAR)"))
        c.execute(text("CREATE TABLE vessels (id INTEGER PRIMARY KEY, organization_id INTEGER, name VARCHAR, registration_no VARCHAR UNIQUE, vessel_type VARCHAR, vessel_class VARCHAR)"))
        c.execute(text("CREATE TABLE declarations (id INTEGER PRIMARY KEY, reference_no VARCHAR UNIQUE, company_name VARCHAR, vessel_name VARCHAR)"))
        c.execute(text("CREATE TABLE audit_events (id INTEGER PRIMARY KEY, entity_type VARCHAR, entity_id INTEGER, action VARCHAR, summary TEXT, actor_user_id INTEGER, organization_id INTEGER, correlation_id VARCHAR, created_at VARCHAR)"))
    ...
    with create_engine(url).connect() as c:
        tables = set(r[0] for r in c.execute(text("select name from sqlite_master where type='table'")))
    ...
    with create_engine(url).connect() as c:
        ...
        assert "reporting_unit_id" not in {r[1] for r in c.execute(text("PRAGMA table_info(audit_events)"))}
        assert c.execute(text("select name from organizations where id=7")).scalar() == "Legacy Co"
        assert c.execute(text("select action from audit_events where id=1")).scalar() == "LOGIN_SUCCESS"
...
def test_schema_drift_missing_secondary_composite_is_rejected(tmp_path):
    ...
    with engine.begin() as c:
        c.execute(text("DROP TABLE historical_cargo_rows"))
        c.execute(text(
...
def test_schema_verification_passes_on_correct_schema(tmp_path):
